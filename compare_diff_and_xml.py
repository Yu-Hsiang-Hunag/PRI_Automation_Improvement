# ==================================================================================================================== #
# Script Name: compare_diff_and_xml
# Usage: -u <JIRA account|string> -o <JIRA account password|xml> -j <JIRA ticket/log_file>
# Example: -u "jahuang" -p "pwd" -j "OEMPRI-25143"
...
try:
    ...
    logging.error("Please Install unidiff")
try:
...
# -------------------------------------------------------------------------------------------------------------------- #
# Reading input file "Field and Value" for comparing to output file
# -------------------------------------------------------------------------------------------------------------------- #
def excel_info(excel_path, output_path):
    ...
    if '.diff' in output_path: # diff file compare
        print("Diff comparing start")
        compare_to_diff_file_info(output_path, search_Field, search_value)
    else:
        print("XML comparing start") # xml file compare
...
def compare_to_diff_file_info(diff_path, s_key, s_val):
    ...
    for i in range(len(s_key)):
        ...
        if s_val[i] is None: # pass Field' value is empty
...
def jira_ticket(username, passwd, ticket):
    ...
    # output file path
    xml_file_path = os.path.join(folder_xml_all, target_file_name+".xml")
    diff_file_path = os.path.join(folder_diff, "Differences-" + target_file_name + ".xml.diff")
    ...
    # Upload to JIRA ticket -- log file and comment
    issue.add_attachment(log_file_path) # upload to JIRA log file
    with open(log_file_path) as f: # open log file for uploading log to JIRA comment
        content = f.read()
    f.close() # close log file
    
    issue.add_comment("{noformat}\n" + content + "\n{noformat}") # add the comment to JIRA ticket
    
    # Remove file download and create
